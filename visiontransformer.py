...
class Block(nn.Module):
    ...
    def forward(self, x, keep_rate=None, get_idx=True):
        if keep_rate is None:
            keep_rate = self.keep_rate  # this is for inference, use the default keep rate
        B, N, C = x.shape

        #x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x))))
        tmp, index, idx, cls_attn, left_tokens = self.attn(self.norm1(x), keep_rate)
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, keep_rate=None, get_idx=True):
        _, _, h, w = x.shape
    ...
    def forward(self, x, keep_rate=None, get_idx=True):
        x, _, idxs = self.forward_features(x, keep_rate, get_idx)
